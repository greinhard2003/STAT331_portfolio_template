...
ca_childcare |>
  ...
  labs(title = "Weekly Median Price for Center-Based Childcare ($)",
       x = "Study Year",
       y = NULL,
       color = str_to_title("Census Region")) +
  theme_bw() +
...
unformatted_tbl <- evals_fct |>
  mutate(
    ...
    cols = c(seniority, sex, academic_degree),
    names_to = "variable",
    values_to = "level"
  ) |> group_by(
    variable,
...
mycifun <- function(beta0, beta1, n){
  stopifnot(is.numeric(beta0))
  stopifnot(is.numeric(beta1))
  ...
  tidy_model = tidy(conf.int = TRUE, conf.level = 0.95, x = lm_model)
  tidy_model |> 
    mutate(cover = ifelse(term == "x" & beta1 >= conf.low & beta1 <= conf.high, 1, 0)) |>
    filter(
      term == "x"
    ) |>
    select(
...
ggplot(data = teacher_evals_compare, 
  ...
  geom_text(
    aes(label = scales::label_percent(accuracy = 1)(after_stat(prop)),
        group = SET_level),
    stat = "count",
...
#| label: dvs-3-1-ex-2
#Lab 8 Question 4
style_plot <- function(plot, theme_fn = theme_minimal, palette = RColorBrewer::brewer.pal(8, "Set2"), aesthetic){
  ...
  if(aesthetic == "fill"){
    ...
    stop("Usage: aesthetic must be 'color' or 'fill'")
  }
  return(result)
}
```
...
ggplot(data = teacher_evals_compare, 
  ...
  geom_bar(position = "fill") +
  geom_text(
    aes(label = label_percent(accuracy = 1)(after_stat(prop)),
        group = SET_level),
    stat = "count",
    position = position_fill(vjust = 0.5),
    color = "white") +
...
ggplot(aes(y = sim_num, x = estimate)) +
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high, color = factor(cover)), height = 0.3, linewidth = 0.8) +
  geom_vline(xintercept = 0.5, linetype = "dashed") +
  scale_color_manual(
    values = c("0" = "red", "1" = "skyblue"),
    labels = c("Does not include true slope", "Includes true slope"),
    guide = "none"
  ) +
  labs(
    title = "95% Confidence Intervals of Estimated Regression Slope\nfor 1000 Simulated Datasets",
    subtitle = "CIs that do not include the population slope are highlighted in red",
...
rescale_01 <- function(vec) {
  if(!is.numeric(vec)){
    stop("Vector type must be numeric")
  }
  if(length(vec) <= 1){
    stop("Vector must contain at least two elements")
  }
...
make_boxplot <- function(df, x, y){
  stopifnot(is.data.frame(df))
  
  x_name <- as_label(enquo(x))
  ...
  x_lab <- reformat(x_name)
  y_lab <- reformat(y_name)
  
  title <- str_c(y_lab, "by", x_lab, sep = " ") |>
    str_remove("\\(mm\\)\\ ")
...
randomBabies <- function(n_babies){
  stopifnot(is.numeric(n_babies))
  babies <- 1:n_babies
  parents <- sample(babies)
  sum(parents == babies)
}
...
The biggest way I have revised my thinking throughout the course is being able to look deeper into data and where it comes from. Initially I was just looking at datasets as tables, and looked at the data inside them very literally. The deepest I would look was something like "Column A is type X". Over time I have started to notice that some columns have connections with eachother or other tables. I have started to see that the dataset as a whole isn't enough to determine what the data is really about. I started looking into the origins of the dataset and began to understand the data I was working with at a deeper level which allowed me to work with it more effectively. The second way I have revised my thinking is by realizing that an output doesn't always mean I am getting the information I want. This finally clicked while I was revising lab 3 and I saw that I counted more teachers than there actually were when trying to investigate academic_degree, seniority, and sex. Initially I missed this because I wasn't focused on the number of teachers as much as I was trying to get the table to show a percentage so I missed the error. This taught me to look over my code more closely and check to make sure that everything is yielding results that actually make sense.
...
The main way I have extended my thinking is by exploring what ggplot2 has to offer. I have spent the most time researching how to tweak the colors of the different parts of my plots. I find it a little difficult to be creative with my plots so I have been trying to force myself to play with the colors in order to get better at it. The other way I have extended my thinking is by using the "?function_name()" functionality within Rstudio in order to learn about a new function or remind myself how to use one instead of googling the function or asking ChatGPT what it does. I have found that the built in documentation is usually enough if not more than I need to figure out how the function works and what it will allow me to do. This has also helped me avoid superseded and deprecated functions. I also spent a decent amount of time researching the capabilities of the gt package after that was introduced. I would have liked to spend more time working with it in class and develop skills with it because I find a lot of times where it would be useful to have a nice table to show some hard data in code reviews. 

## Peer Support & Collaboration

<!-- Include an image or a description of feedback you gave that you are proud of (either in a peer review or in Discord). -->

![I am proud of this feedback because I noticed a subtle issue with their answer that affected the correctess of their plot and statistical analysis. I tend to miss small issues like these myself and it is hard to debug sometimes, so I try to catch them in others peoples work so that they have an easier time debugging their code later.](Feedback_Image.png)

<!-- Include a description of how you grew as a collaborator through the weekly pair programming activities.   -->

During the pair programming activities I have learned how to explain my ideas to my partner better as the weeks have progressed. Initially I found it hard to to work effectively because I couldn't explain why something would work to someone that isn't as experienced with programming as I am. I found that giving examples along with my proposed solution allowed my partner to better understand why we were doing something rather than just moving on because we got the correct answer. I think this is an important skill to keep improving on because being a more effective team member will eventually make it so that any team I am a part of will work more efficiently.
